...
@app.get("/usuarios/{dni}/obras", response_model=List[ObraResponse])
def obras_de_usuario(dni: str, db: Session = Depends(get_db)):
    asignaciones = db.query(Asignacion).filter(Asignacion.dni_usuario == dni, Asignacion.activo == True).all()
    if not asignaciones:
        return []
    ids = [a.id_obra for a in asignaciones]
    obras = db.query(Obra).filter(Obra.id_obra.in_(ids)).all()
    return obras

@app.get("/me/obras", response_model=List[ObraResponse])
def mis_obras(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    asignaciones = db.query(Asignacion).filter(Asignacion.dni_usuario == current_user.dni, Asignacion.activo == True).all()
    ids = [a.id_obra for a in asignaciones]
    return db.query(Obra).filter(Obra.id_obra.in_(ids)).all()


@app.get("/me/asistencias", response_model=List[AsistenciaResponse])
def mis_asistencias(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    return db.query(Asistencia).filter(Asistencia.dni_usuario == current_user.dni).all()
